...
from app.models import Course, Session_Year, CustomtUser, Staff, Student, Subject, Staff_Notification, Staff_Leave
...
def ADD_STUDENT(request):
    ...
    if request.method == "POST":
        ...
        if CustomtUser.objects.filter(email=email).exists():
            messages.warning(request, 'Email Already Taken')
            return redirect('add_student')
        if CustomtUser.objects.filter(username=username).exists():
            ...
            messages.success(request, user.first_name + " " +
                             user.last_name + " Successfully Add")
            return redirect('add_student')
...
def UPDATE_STUDENT(request):
    if request.method == "POST":
        ...
        user = CustomtUser.objects.get(id=student_id)
...
def DELETE_STUDENT(request, admin):
    student = CustomtUser.objects.get(id=admin)
    student.delete()
...
def ADD_COURSE(request):
    if request.method == "POST":
        ...
        course.save()
        messages.success(request, course_name + " " + " Successfully Add")
...
def UPDATE_COURSE(request):
    if request.method == "POST":
        ...
        course.save()
        messages.success(
            request, 'Course Successfully Updated (' + course_id + ')')
...
def UPDATE_STAFF(request):
    if request.method == "POST":
        ...
        user = CustomtUser.objects.get(id=staff_id)
...
def DELETE_STAFF(request, admin):
    staff = CustomtUser.objects.get(id=admin)
    staff.delete()
...
@login_required(login_url='/')
def STAFF_APPROVE_VIEW(request, id):
...
@login_required(login_url='/')
def STAFF_DISAPPROVE_VIEW(request, id):
    leave = Staff_Leave.objects.get(id=id)
    leave.status = 2
    leave.save()
    return redirect('staff_leave_view')
